...
ll max_value_using_rectangles(int factor1,int factor2,int max_value_enclosed,int no_of_rectangles_chosen,pair<int,int>&best_case_divisions,vector< vector<int> >&best_case_rects,vector<vector<int>>&pre)
{
...
vector<pair<pair<float,float>,pair<float,float>>> func(int reverse,vector<vector<int>>best_case_rects,pair<int,int>best_case_divisions)
{
...
int main()
{
    ...
    for (int iter = 0; iter < 10; iter++)
    {
        ...
        for(auto factor1 : factors)
        {
            ...
            {
                if(factor1>=200)
                    max_value_enclosedy=max(max_value_enclosedy,max_value_using_rectangles(factor1,factor2,max_value_enclosedy,160,best_case_divisionsy,best_case_rectsy,prefix_vector_2d));
                else
                    max_value_enclosedy=max(max_value_enclosedy,max_value_using_rectangles(factor1,factor2,max_value_enclosedy,200,best_case_divisionsy,best_case_rectsy,prefix_vector_2d));

                if(factor2>=200)
                    max_value_enclosedx=max(max_value_enclosedx,max_value_using_rectangles(factor2,factor1,max_value_enclosedx,160,best_case_divisionsx,best_case_rectsx,prefix_vector_2d));
                else
                    max_value_enclosedx=max(max_value_enclosedx,max_value_using_rectangles(factor2,factor1,max_value_enclosedx,200,best_case_divisionsx,best_case_rectsx,prefix_vector_2d));
            }
        }
        vector<pair<pair<float,float>,pair<float,float>>> edges,edges_x,edges_y;
        edges_x=func(0,best_case_rectsx,best_case_divisionsx);

        swap(best_case_divisionsy.first,best_case_divisionsy.second);
        edges_y=func(1,best_case_rectsy,best_case_divisionsy);
